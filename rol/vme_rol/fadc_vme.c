...
rocDownload()
{
  ...
  int ifa;
  char fadc_config_file[512];  /* Path to FADC config file to use */

  printf("***** rocDownload() ENTERED - START OF FUNCTION *****\n");
  fflush(stdout);

  /* ===================================================================
   * CONFIG FILE SELECTION (NO RUNTIME PEDESTAL/CONFIG GENERATION)
   * =================================================================== */
  {
    const char *coda_config_env;

    printf("INFO: ============================================\n");
    printf("INFO: Determining FADC configuration file\n");
    printf("INFO: ============================================\n");

    /* Get CODA_CONFIG environment variable */
    ...
    if (access(fadc_config_file, R_OK) != 0)
    {
      printf("ERROR: rocDownload - Config file '%s' not found or not readable: %s\n",
             fadc_config_file, strerror(errno));
      printf("ERROR: rocDownload - DOWNLOAD TRANSITION FAILED\n");
      return;
    }

    printf("INFO: Config file verified: %s\n", fadc_config_file);
    printf("INFO: ============================================\n");
  }
  /* ===================================================================
  ...
  printf("***** CONFIG FILE SELECTION COMPLETED - CONTINUING WITH FADC INIT *****\n");
  fflush(stdout);
  ...
  /* Read in the FADC250 Config file (selected above) */
  printf("INFO: ============================================\n");
  printf("INFO: Configure FADC using config file\n");
  printf("INFO: ============================================\n");
  printf("INFO: Using config file: %s\n", fadc_config_file);
  stat = fadc250Config(fadc_config_file);
  if(stat<0) {
    printf("ERROR: Reading FADC250 Config file '%s' FAILED\n", fadc_config_file);
  }else{
    printf("INFO: Successfully loaded FADC250 Config file\n");
  }
  ...
  /* VTP hardware configuration is handled by VTP ROL (not VME ROL) */
  printf("INFO: VTP configuration skipped (handled by VTP ROL)\n");
