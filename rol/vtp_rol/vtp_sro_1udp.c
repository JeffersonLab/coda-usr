...
rocDownload()
{
  ...
  if (!coda_firmware || !*coda_firmware) {
    ...
    if (!coda || !*coda) {
      ...
      fprintf(stderr, "ERROR: Cannot determine firmware directory\n");
      return;
    }
    /* Use CODA/firmware as fallback */
  ...
  /* Load firmware using $CODA_FIRMWARE directory */
  ...
  {
    printf("ERROR: Z7 programming failed: %s\n", buf);
    return;
  }
  printf("INFO: Z7 firmware loaded successfully\n");

  snprintf(buf, sizeof(buf), "%s/%s", coda_firmware, v7file);
  printf("INFO: Loading V7 firmware: %s\n", buf);

  if(access(buf, R_OK) != 0)
  {
    printf("ERROR: V7 firmware file not found or not readable: %s\n", buf);
    printf("ERROR: Check CODA_FIRMWARE environment variable and firmware file\n");
    return;
  }
  ...
  {
    printf("ERROR: V7 programming failed: %s\n", buf);
    return;
  }
  printf("INFO: V7 firmware loaded successfully\n");
...
rocPrestart()
{
  ...
  /* Use userconfig if defined, otherwise use default per-host config file */
  {
    ...
    else
    {
      ...
      {
        printf("ERROR: CODA_CONFIG environment variable not set\n");
        printf("ERROR: Cannot determine VTP config file location\n");
        return;
      }

      /* Construct default config path using helper function */
      if (vtp_get_generated_config_path(vtp_config_path, sizeof(vtp_config_path)) != 0)
      {
        printf("ERROR: Failed to construct default VTP config path\n");
        printf("ERROR: Cannot determine vtp_<hostname>.cnf location\n");
